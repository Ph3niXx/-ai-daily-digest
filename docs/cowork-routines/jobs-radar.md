...
job_pref_rules = regles ecrites par Jean : AUTORITE ABSOLUE, ne jamais les contredire. job_pref_observed = typologie inferee de ses verdicts, sous forme de PENALITES CHIFFREES a appliquer (voir Etape 3). skill_radar + user_profile = le PROFIL de Jean, pour le match des skills (on_cv).
...
- score_total = somme des 3 axes, PUIS application du CALIBRAGE ci-dessous, arrondi 1 decimale, borne 0 a 10.

CALIBRAGE — ETAPE OBLIGATOIRE ET CHIFFREE, PAS UN COMMENTAIRE.
job_pref_rules prime sur tout. job_pref_observed (lu a l'Etape 0) contient une table de PENALITES CHIFFREES derivee des verdicts de Jean : applique-la au score_total apres avoir pose les 3 axes, en CUMULANT toutes les lignes qui s'appliquent, et NOMME CHAQUE PENALITE AVEC SON MONTANT dans la cle calibrage (exemple : 'TPM technique -2.5 ; defense -2.0'). Si une ligne VETO s'applique, plafonne score_total a 2.0 ET mets status='archived', quelle que soit la qualite intrinseque du poste.
POURQUOI c'est imperatif : les 15 rejets de Jean portent TOUS sur des offres que la rubric avait notees 7.5 a 10. La rubric mesure la qualite du POSTE ; Jean juge sa CREDIBILITE dessus. Ecrire l'objection sans bouger le chiffre est le mode d'echec exact constate le 2026-07-28 — un Technical Program Manager Defense chez Cohere portait la mention « resonne avec rejet Mistral TPM. Score ajuste » et sortait quand meme a 8.0 en 'new'. Une cle calibrage sans montant numerique signifie que le test n'a pas ete fait.
...
SORTIE : affiche un resume court — SANTE DU FETCH en premier (lot de rotation, requetes ok/echouees, codes HTTP, quota restant sur le mois, aborted_reason), puis nombre d'offres fetchees / ecartees (non-FULLTIME) / dedupliquees / archivees / vieillies / hot leads, le nombre d'offres ayant subi une PENALITE de calibrage et combien ont ete VETO, l'etat du recalibrage (fait / saute et pourquoi), et le Top 3 (titre, score, ~target k€).
```
