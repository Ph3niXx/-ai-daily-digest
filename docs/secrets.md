...
**Posés le 2026-08-13 (12:35 UTC)** : les deux secrets ont été ajoutés à GitHub Actions et le pipeline a tourné en écriture pour de bon — seed initial de 78 jeux Steam, 24 licences (collections) surveillées et 451 titres au total le 2026-08-13. Sans ces secrets, `igdb_tracker_sync.py` sort en `[skip]` avec le code 0 dès la première ligne (avant tout appel réseau) : le workflow reste vert, zéro écriture, comportement nominal documenté dans `docs/architecture/pipelines.yaml::igdb_tracker_sync.health`.
