...
| `games_unwatch_franchise` | `{franchise, surface}` | `cockpit/home.jsx::GamesBriefCard::unwatch()` **ou** `cockpit/panel-gaming.jsx::PanelGaming::unwatchFranchise()`, après PATCH réussi sur `game_franchises` (`watched: false`) + acquittement des événements — l'utilisateur cesse de suivre la licence. `surface` vaut `brief` ou `gaming` selon l'écran. **Sonde de survie du lot 1** : trois semaines sans `games_release_ack` ni `games_unwatch_franchise` après le premier événement détecté dans `game_releases` ⇒ le lot 2 (pipeline IGDB) n'est pas lancé et l'encart est retiré |
