...
| `games_release_ack` | `{event_type}` | `cockpit/home.jsx::GamesBriefCard::ack()` après PATCH réussi sur `game_releases` (`acknowledged: true`) — l'utilisateur acquitte une annonce/date/sortie/annulation |
| `games_unwatch_franchise` | `{franchise}` | `cockpit/home.jsx::GamesBriefCard::unwatch()` après PATCH réussi sur `game_franchises` (`watched: false`) + acquittement de l'événement — l'utilisateur cesse de suivre la licence. **Sonde de survie du lot 1** : trois semaines sans `games_release_ack` ni `games_unwatch_franchise` après le premier événement détecté dans `game_releases` ⇒ le lot 2 (pipeline IGDB) n'est pas lancé et l'encart est retiré |
