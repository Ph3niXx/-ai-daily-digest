...
## Dernière MAJ
2026-08-21 (addendum) — **nouveau cas de bord : des articles sans brief.** Les etapes 3 a 9 du pipeline de veille sont desormais encapsulees individuellement : un echec de Gemini ne tue plus le run avant l'ecriture des articles (step 7/9). Consequence visible ici — la ou un 503 Gemini laissait auparavant la journee entierement vide, le Top du jour et la liste d'articles peuvent maintenant s'afficher normalement pendant que la synthese du hero manque. Le run sort en erreur et le bandeau de sante le signale, mais le panel reste utilisable au lieu d'etre desert. Ajout au meme moment d'une detection des flux geles (200 avec des items figes depuis plus de 180 jours), qui borne le risque du miroir tiers utilise pour la source Anthropic. Cf. ADR-44.
