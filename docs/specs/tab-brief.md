...
**Repli mobile (sous 760 px).** Le hero (synthèse du jour, "À traiter depuis hier") et la vue Morning Card ("Commence par ça.") restent tels quels, visibles sans clic : c'est ce qui justifie l'ouverture du Brief le matin, et le portage n'y touche pas. Les trois blocs qui suivent passent chacun par `PanelSection` (`cockpit/components-mobile.jsx`) et se replient en `<details>` natif, fermés par défaut : **Top du jour**, **Signaux faibles + Radar compétences**, **Ma semaine** — dans cet ordre. Chacun affiche un `hint` dans son `<summary>` qui résume son contenu sans avoir à déplier (nombre d'incontournables, nombre de signaux, nombre d'articles lus). **Décision éditoriale, pas un détail d'implémentation** : aucun des trois n'est épinglé (`pinned`) — si l'usage mobile montre que l'un d'eux est en réalité consulté systématiquement sans jamais rester replié, il doit rejoindre le hero comme visible par défaut plutôt que de rester un repli qu'on rouvre à chaque fois. L'état "à jour" (zéro article restant, message "Tu as fait le tour. Bravo.") et l'état "zéro signal" ne passent pas par `PanelSection` : ce sont des remplacements complets du bloc Top, pas des sections à replier. Au-dessus de 760 px, `PanelSection` est un passe-plat exact (ADR-46) : aucun changement de DOM, aucune régression desktop.
...
## Dernière MAJ
2026-08-21 — **portage mobile (vague 1) : repli progressif sous 760 px.** Le hero et la vue Morning Card restent visibles ; Top du jour, Signaux faibles + Radar compétences et Ma semaine passent chacun par `PanelSection` (`cockpit/components-mobile.jsx`) et se replient en `<details>` natif fermé par défaut, avec un `hint` qui résume le contenu replié. Aucun des trois n'est épinglé — décision éditoriale, à revoir si l'usage la contredit. Au-dessus de 760 px, `PanelSection` ne change rien au DOM. Voir « Front — structure UI » et ADR-46.
