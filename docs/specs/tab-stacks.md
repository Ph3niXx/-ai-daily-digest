...
| `pipeline_health` | Lu en **Tier 1** (table minuscule, ~19 lignes), exposé sur `COCKPIT_DATA.pipeline_health`. La source de vérité de l'état des pipelines est désormais l'onglet Santé ; ce panel n'en garde qu'un renvoi chiffré, et le bandeau global de `app.jsx` continue d'alerter comme avant. On charge toutes les lignes, pas seulement les dégradées : sans les lignes saines on ne peut pas distinguer « rien à signaler » de « le contrôle est mort et la table a gelé » (arbitrage via `checked_at`). | **19 lignes** (catalogue au 2026-08-20) |
...
## Dernière MAJ
2026-08-20 — la liste complète des pipelines part dans l'onglet [Santé](tab-sante.md) (groupe Coulisses) : `StPipelineHealth` est remplacée par un renvoi `StHealthLink` (compteur de sources dégradées sur le total, clic vers Santé). Étape 4bis du parcours et section « Fonctionnalités » mises à jour en conséquence. Le bandeau global `PipelineHealthBanner` dans [app.jsx](cockpit/app.jsx) gagne un lien « Tout voir » vers Santé, comportement inchangé sinon (mêmes lignes, même tri, même garde 48 h — désormais partagés avec `cockpit/lib/sante-view.js`). Raison complète : ADR-41 dans `docs/architecture/decisions.md`.
