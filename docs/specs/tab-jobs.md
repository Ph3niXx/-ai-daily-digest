...
> Feed d'offres scorées par fit (0-10) via un scan automatisé 3×/semaine (routine Claude Code distante sur API jobs structurée), décomposées en "hot leads" (score ≥ 7) + liste dense filtrée/triable, avec statut (new/to_apply/applied/snoozed/archived) et notes perso persistés en DB, et rafraîchissement temps-réel via Supabase channels.
...
5. Utilisation des filtres : recherche texte + cinq groupes de filtres (score hot/mid/low / rôle / lieu / fraîcheur / statut) + tri (score ou récence). Filtre statut "Actives" par défaut, qui masque les snoozées et archivées. La **fraîcheur** et le tri **récence** se mesurent sur la date d'entrée de l'offre dans le radar (`first_seen_date`), pas la date de publication LinkedIn.
